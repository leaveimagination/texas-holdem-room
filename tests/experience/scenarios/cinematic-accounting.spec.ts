...
test("EXP-004 renders a four-way all-in as a complete cinematic timeline", async ({ browser }) => {
  ...
  await runCase(browser, "EXP-004", async ({ fixture: f, coordinates }) => {
    ...
    const room = await provision(f, fixture, ["aces", "kings", "queens", "jacks"]);
    const spectator = f.pool.get("spectator");
    f.evidenceActor = spectator;
    for (const action of fixture.actionPlan) await perform(room.pages[action.actorRole], action);
    const timelinePromise = collectRunoutTimeline(spectator.page);
    const actionMonitorPromise = Promise.all(["player-1", "player-2", "player-3", "player-4"].map((id) => monitorRunoutActions(f.pool.get(id).page)));
    const timeline = await timelinePromise;
    const playerActionMaxima = await actionMonitorPromise;
...
test("EXP-005 matches independent side-pot and split-pot oracles", async ({ browser }) => {
  ...
  await runCase(browser, "EXP-005", async ({ fixture: f, coordinates }) => {
    ...
    const sideRoom = await provision(f, side, ["aces", "kings", "queens", "jacks"]);
    for (const action of side.actionPlan) await perform(sideRoom.pages[action.actorRole], action);
    const sideResult = await readHandResult(f.pool.get("spectator").page);
    ...
    const splitRoom = await provision(f, split, ["left", "right"]);
    for (const action of split.actionPlan) await perform(splitRoom.pages[action.actorRole], action);
    const splitResult = await readHandResult(f.pool.get("spectator").page);
...
test("EXP-006 accumulates queued chips and applies them once at the next hand", async ({ browser }) => {
  ...
  await runCase(browser, "EXP-006", async ({ fixture: f, coordinates }) => {
    ...
    const room = await provision(f, oracle, ["target", "opponent"]);
    ...
    const observedNotifications = [await Promise.all(notificationPages.map(async (page) => { await page.getByText(/queued 300 chips/).waitFor({ state: "visible" }); return 300; }))];
    await room.pages.target.queueTopUp(200);
    observedNotifications.push(await Promise.all(notificationPages.map(async (page) => { await page.getByText(/queued 200 chips/).waitFor({ state: "visible" }); return 200; })));
    await target.page.locator('[data-pending-top-up="500"]').waitFor({ state: "visible" });
    ...
    const settled = await readHandResult(target.page);
    ...
    await target.page.locator('[aria-label="Table"][data-flow-phase="betting"][data-hand-number="2"]').waitFor({ state: "visible", timeout: 8_000 });
    ...
    await delay(300);
    product(await stackFor(target.page, room.ids.target) === next, coordinates, "EXP-006-A05", "target", "stable", next);
...
test("EXP-007 preserves hand and session accounting through a live-hand end request", async ({ browser }) => {
  ...
  await runCase(browser, "EXP-007", async ({ fixture: f, coordinates }) => {
    ...
    const handOne = await handResultPromise;
    product(handOne.result.players.length === 2 && handOne.result.players.every((p) => p.text.includes("→") && /[+-]?\d/.test(p.net)), coordinates, "EXP-007-A01", "spectator", handOne.result.players, "all start/end/signed net rows");
    ...
    await f.pool.get("host").page.locator('[aria-label="Table"][data-flow-phase="betting"][data-hand-number="2"]').waitFor({ state: "visible", timeout: 8_000 });
    ...
    await room.pages.host.waitForPhase("session-summary", { timeout: 8_000 });
...
async function provision<Role extends string>(f: Fx, poker: PokerFixture<Role, any, any>, roles: readonly Role[]) {
  ...
  await roomPage(spectator).waitForPhase("betting", { timeout: 5_000 }); return { pages, names, ids };
}

interface Frame { kind: "showdown" | "board" | "settlement"; at: number; boardLength: number; enabledActions: number }
async function collectRunoutTimeline(page: Page): Promise<Frame[]> { return await page.evaluate(async () => await new Promise<Frame[]>((resolve, reject) => { const start = performance.now(); const frames: Frame[] = []; let lastBoard = 0; let showdown = false; const deadline = start + 20_000; const tick = () => { const table = document.querySelector<HTMLElement>('[aria-label="Table"]'); const phase = table?.dataset.flowPhase ?? ""; const board = Number(table?.dataset.boardCardCount ?? 0); const enabled = document.querySelectorAll('[data-action-type]:not([disabled])').length; if (phase === "showdown-reveal" && !showdown) { showdown = true; frames.push({ kind: "showdown", at: performance.now() - start, boardLength: board, enabledActions: enabled }); } if (board !== lastBoard) { frames.push({ kind: "board", at: performance.now() - start, boardLength: board, enabledActions: enabled }); lastBoard = board; } if (phase === "hand-summary") { frames.push({ kind: "settlement", at: performance.now() - start, boardLength: board, enabledActions: enabled }); resolve(frames); return; } if (performance.now() > deadline) { reject(Object.assign(new Error("runout timeline timeout"), { name: "TimeoutError" })); return; } requestAnimationFrame(tick); }; requestAnimationFrame(tick); })); }
async function monitorRunoutActions(page: Page): Promise<number> { return await page.evaluate(async () => await new Promise<number>((resolve) => { let maximum = 0; const tick = () => { maximum = Math.max(maximum, document.querySelectorAll('[data-action-type]:not([disabled])').length); if (document.querySelector<HTMLElement>('[aria-label="Table"]')?.dataset.flowPhase === "hand-summary") { resolve(maximum); return; } requestAnimationFrame(tick); }; requestAnimationFrame(tick); })); }
...
async function watchHandResult(page: Page) { return await page.evaluate(async () => await new Promise<{ result: { players: Array<{ name: string; text: string; net: string; end: number }>; pots: Array<{ amount: number; awards: string }> }; visibleMs: number }>((resolve) => { let shownAt: number | null = null; let captured: { players: Array<{ name: string; text: string; net: string; end: number }>; pots: Array<{ amount: number; awards: string }> } | null = null; const tick = () => { const root = document.querySelector<HTMLElement>('[aria-label="Hand result"]'); if (root && shownAt === null) { shownAt = performance.now(); captured = { players: Array.from(root.querySelectorAll('.hand-result-player')).map((p) => ({ name: p.querySelector('span')?.textContent?.trim() ?? "", text: p.querySelector('small')?.textContent?.trim() ?? "", net: p.querySelector('strong')?.textContent?.trim() ?? "", end: Number((p.querySelector('small')?.textContent ?? "").split('→')[1]?.replaceAll(',', '').trim()) })), pots: Array.from(root.querySelectorAll('.hand-result-pots > div')).map((p) => ({ amount: Number((p.querySelector('span')?.textContent ?? '').match(/[\d,]+$/)?.[0].replaceAll(',', '')), awards: p.querySelector('strong')?.textContent ?? "" })) }; } if (!root && shownAt !== null && captured) { resolve({ result: captured, visibleMs: performance.now() - shownAt }); return; } requestAnimationFrame(tick); }; requestAnimationFrame(tick); })); }
