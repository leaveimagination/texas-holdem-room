...
Each finalized run writes aggregate `case-manifest.json`, `events.json`, `report.json`, `report.html`, and `diagnostics/` under `outputs/site-test/<runId>/`. Scenarios write per-attempt screenshots, videos, traces, and visual diffs directly under `outputs/site-test/<runId>/cases/<caseId>/<attemptId>/`; this finalized run directory is the canonical evidence pack. The recorder separately writes structured attempt reports and events to the run-exact `.case-evidence-<runId>` sidecar, which the collector reads during aggregation. That sidecar may remain after finalization, is non-canonical, and must not be shared instead of the validated pack. Retain it for diagnosis or remove only that exact sidecar after the canonical pack validates. Partial evidence is made durable before cleanup. Known credentials and private payloads are redacted and the final validator checks ordinary files and decompressed trace entries.
